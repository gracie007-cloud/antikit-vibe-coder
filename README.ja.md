...
- ⚠️ **英語ばかり:** 日本語で聞いても、英語（または不自然な日本語）で返ってくる。
...
> **💡 ヒント:** 言語はいつでも変更できます: `/config language [en|vi|ja|zh]`
...
2.  **🧠 44の専門スキル:** エージェントには以下の深い知識がプリロードされています：
    *   **技術:** Next.js, React, Node.js, Python, Rust, Docker, Cloudflare.
    *   **方法論:** TDD, Clean Code, DDD, セキュリティパターン.
    *   **専門分野:** SEO, UX/UI, パフォーマンス分析, モバイル, ゲーム開発.
3.  **⚡ パワーモードフラグ:** シンプルなフラグで専門エージェントをアンロック：
    *   `/code --mobile` → React Nativeスキルのモバイルエキスパート
    *   `/audit --pentest` → MITRE ATT&CKのペネトレーションテスター
    *   `/deploy --docker` → DockerスキルのDevOpsエキスパート
4.  **👮 規律の強制:** チーム全体で **Linting、Testing、Safety** のルールを強制できます。
5.  **🧘 ディープ・フロー:** AIがバックグラウンドで安全に作業を処理します。

---

## 📦 箱の中身は？

-   **20の専門エージェント:** フロントエンド、バックエンド、セキュリティ、SEO、モバイル、ゲーム開発...
-   **29 ワークフロー:** 4言語対応のスラッシュコマンド、**パワーモードフラグ**付き。
-   **44 スキル:** オンデマンドでロードされる専門知識モジュール。
-   **グローバルメモリ (GEMINI.md):** 安全、言語、ルーティングを強制する「頭脳」
