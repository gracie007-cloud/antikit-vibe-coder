...
➡️ **下一步:**
1️⃣ 开始阶段2? `/code phase-02`
2️⃣ 休息一下? `/save-brain` 保存进度"
```

---

## 阶段 6: 🔍 预提交检查清单 & 破坏性变更检测
...
✅ 预提交检查清单:
□ 代码符合用户的要求吗？
□ 是否添加了要求之外的内容？（如果是 → 询问用户）
□ 输入验证是否完整？
□ try-catch错误处理是否完整？
□ 是否硬编码了敏感数据？（API密钥、密码）
□ 是否留有调试console.log？（提交前删除）
□ 复杂逻辑是否需要注释？
□ 变量/函数名称是否清晰？
```
...
### 重大变更后:
*   "这是重要变更。记得在会话结束时 `/save-brain` ！"

### 发现有用模式时:
*   "💡 我注意到这个 [X] 模式很有用。要保存到GLOBAL吗？"
...
```
1️⃣ 继续阶段中的下一个任务
2️⃣ 进入下一阶段? `/code phase-XX`
3️⃣ 检查进度? `/next`
4️⃣ 保存上下文? `/save-brain`
```
